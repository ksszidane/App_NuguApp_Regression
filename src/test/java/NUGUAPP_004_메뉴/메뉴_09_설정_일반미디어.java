...
public class 메뉴_09_설정_일반미디어 extends TestCase {
	...
	@Test(description = "누구앱 리그레이션 TC : 메뉴_121")
	public void TC_메뉴_122(Method method) throws Exception {
	    ...
	    Assert.assertEquals(T아이디, "우선 재생 음악 서비스를 설정해주세요.");
		...
		for(int i=1; i < 11; i++) {
	    	...
	    	+"/android.widget.RelativeLayout["+i+"]/android.widget.TextView[1]"));
		    System.out.println(i + "번 : " + 디바이스목록);
		    	
		    Assert.assertTrue(디바이스목록.contains(NUGU_data.data.연결된디바이스목록[i-1]));
	    }
